...
const SignUp=()=>{
  ...
  const User = z.obect({
    name: z.string().min(3, {message: "Must be 3 or more character long"}),
    phoneNumber: z.string().length(10, {meassge: "Must be greater than 10 digits"}),
    dob: z.string().date(),
  ...
  const {registration, handleSubmit, formState: { errors}} = useForm({resolver: zodResolver(User)});
  ...
  return(
    <div className="Sign_Card">
      ...
      <form onSubmit={handleSubmit(handleForm)}>
        ...
        {errors.email && <span className="signUpError">
          ...
          <input placeholder="PhoneNumber" type="number" name="phone Number" autoComplete="off" {...register("phoneNumber")}/>{errors.phoneNumber && <span className="signUpError">
